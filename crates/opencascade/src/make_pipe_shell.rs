...
pub(crate) fn make_pipe_shell_with_law_function(
    profile: &ffi::TopoDSWire,
    spine: &ffi::TopoDSWire,
    law_function: &ffi::HandleLawFunction,
) -> UniquePtr<ffi::BRepOffsetAPIMakePipeShell> {
    let mut make_pipe_shell = ffi::BRepOffsetAPIMakePipeShell_ctor(spine);
    make_pipe_shell.pin_mut().set_mode(false);
